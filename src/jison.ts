...
// async function EsmMain(args) {
//   if (!args[1]) {
//     console.log('Usage: ' + args[0] + ' FILE');
//     process.exit(1);
//   }
//   const source = (await import('fs')).readFileSync(
//     (await import('path')).normalize(args[1]),
//     'utf8'
//   );
...
Productions:
    ...
    while (cont) {
      ...
      productions.forEach(function Follow_prod_forEach(production, k) {
        ...
        for (var i = 0, t; (t = production.handle[i]); ++i) {
          ...
          ItemSet.union(nonterminals[t].follows, set);
    ...
    if (symbol === '') {
      ...
      for (var i = 0, t; (t = symbol[i]); ++i) {
        if (!this.nonterminals[t]) {
          ...
          ItemSet.union(firsts, this.nonterminals[t].first);
        }
    ...
    while (cont) {
      ...
      for (symbol in nonterminals) {
        ...
        nonterminals[symbol].productions.forEach(function (production) {
          ItemSet.union(firsts, production.first);
        });
  ...
  lrGeneratorMixin.ItemSet = ItemSet.prototype.construct({
  ...
  lrGeneratorMixin.canonicalCollectionInsert = function canonicalCollectionInsert(
    symbol,
    itemSet,
    states,
    stateNum
...
parser = ${moduleName};
const Parser = ${moduleName}.Parser;
const parse = ([...args]) => ${moduleName}.parse.apply(${moduleName}, args);
const main = ${String(opt.moduleMain || function () {}) /* || EsmMain)};*/}

export { parser, Parser, parse, main };
...
if (typeof require !== 'undefined' && typeof exports !== 'undefined') {
  ...
  lrGeneratorMixin.generateModule = function generateModule(opt) {
    opt = typal.mix.call({}, this.options, opt);
    var moduleName = opt.moduleName || 'parser';
    const out = `/* parser generated by jison ${version} */
...
Parser: {
  ...
  token location info (@$, _$, etc.): {
    ...
    first_column: n,
    last_column: n,
    range: [start_number, end_number]  (where the numbers are zero-based indexes into the input string)
  }
...
${moduleName.match(/\./) ? moduleName : 'var ' + moduleName} = ${this.generateModuleExpr()}`;
...
`;
  ...
  lrGeneratorMixin.generateModule_ = function generateModule_() {
    ...
    moduleCode += [
      ...
      'parseError: ' +
        String(this.parseError || (this.hasErrorRecovery ? traceParseError : parser.parseError)),
      'parse: ' + parseFn,
    ].join(',\n');
    moduleCode += '};';
  ...
  var createObjectCode =
    'o=function(k,v,o,l){' + 'for(o=o||{},l=k.length;l--;o[k[l]]=v);' + 'return o}';
      ...
      states.forEach(function union_states_forEach(i) {
        ...
        if (state.reductions.length)
          state.reductions.forEach(function union_reduction_forEach(item) {
            ...
            state.goes[item.production.handle.join(' ')].forEach(function reduction_goes_forEach(
              symbol
            ) {
   ...
   * SLR Parser
   * */
  SLRGenerator = lrLookaheadGenerator.construct({
    type: 'SLR(1)',
